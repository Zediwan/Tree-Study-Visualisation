...
(function (global, factory) {
    ...
    var sankey = function() {
        ...
        function moveSinksRight(graph, depth) {
            graph.nodes.forEach(function(node) {
                if (!node.sourceLinks.length && node.value > 0) {
                    node.depth = depth - 1;
                }
                /* modified by Pascal Gerig and Elias Wipfli, if the value of a node is 0 or less then send it out of the sankey
                 *  with the max depth*/
            ...
            function initializeNodeBreadth() {
                ...
                nodesByDepth.forEach(function(nodes) {
                    nodes.forEach(function(node, i) {
                        //again modified for handling the empty nodes
                        if(node.value > 0)
                        {
                        ...
                        {
                            var maxIndexInDepth = nodesByDepth[nodesByDepth.length - 1][nodesByDepth[nodesByDepth.length - 1].length - 1].index;
                            node.y1 = (node.y0 = maxIndexInDepth + 1) + node.value * ky;
                        }
                    });
                });
